...
public class ConsoleApplication implements CommandLineRunner {
	...
	public void run(String... args) {
		...
		log.info("--------------------------------");
		Product first = new Product("gear-surf-surfboards", "Sunnox Surfboard", 8, true);
		Product first2 = repository.save(first);
		log.info(first2.toString());
		log.info("--------------------------------");

		log.info("Create second document");
		log.info("--------------------------------");
		Product second = new Product("gear-surf-surfboards", "Noosa Surfboard", 15, false);
		...
		log.info("Create second document");
		log.info("--------------------------------");
		Product third = new Product("Boat", "Plorif Surf Wetsuit", 2, false);
		...
		if (match.isPresent()) {
			 ...
			 * log.info("--------------------------------");
			 * log.info("delect a specific document"); repository.deleteById(third.id);
			 * 
			 * log.info("Query all documents");
			 * log.info("--------------------------------");
			 * 
			 * for (Product product : repository.findAll()) { log.info(product.toString());
			 * }
			 */
